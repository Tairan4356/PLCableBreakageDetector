...
class FeaturePyramidSharedConv(nn.Module):
    ...
    def forward(self, x):
        ...
        for dilation in self.dilations:
            y.append(
                F.conv2d(y[-1], weight=self.share_conv[0].weight, bias=None, dilation=dilation, groups=y[-1].shape[1],
                         padding=(dilation * (3 - 1) + 1) // 2))

        for dilation1 in self.dilations1:
            y1.append(
                F.conv2d(y1[-1], weight=self.share_conv[0].weight, bias=None, dilation=dilation1, groups=y[-1].shape[1],
                         padding=(dilation1 * (3 - 1) + 1) // 2))

        for dilation2 in self.dilations2:
            y2.append(
                F.conv2d(y2[-1], weight=self.share_conv[0].weight, bias=None, dilation=dilation2, groups=y[-1].shape[1],
...
class C2f_DySnakeConv_custom(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            Bottleneck_DySnakeConv_custom(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
